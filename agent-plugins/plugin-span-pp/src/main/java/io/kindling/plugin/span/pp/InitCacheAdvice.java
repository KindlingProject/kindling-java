...
public class InitCacheAdvice implements BeforeAdvice {
    private final AdviceConfig ADVICE_CONFIG;

    public InitCacheAdvice() {
        ADVICE_CONFIG = new AdviceConfig().enableThisParam();
    }

    public void before(JoinPoint joinPoint) {
        if (PpApiCache.checkAndCache()) {
            PpApiCache.cache((DefaultTraceContext) joinPoint.getThis());
        }
    }
